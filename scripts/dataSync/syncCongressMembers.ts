...
export class CongressMembersSync {
  ...
  public async syncAllMembersFromPropublica (congress: number) {
    ...
    let sync = async (m: MembersAPITypes.GetMemberListAPI.Member) => {
      if (syncedMemIdx.has(m.id)) {
        fLog.log(`Skip duplicate & already synced member id = ${m.id}\n\n`);
        return;
      }
    ...
    await this.g.insertEntities(batchInsert);

    fLog.log(`-------------- Batch Updating (${batchUpdate.length} items) --------------`);
    await this.g.updateEntities(batchUpdate);
  }
  ...
  public async syncMemberFromPropublica (
    ...
    let ents = <dbLib2.IEntPerson[]> await this.g.findEntities({
    ...
    if (ents.length === 0) {
      ...
      batchInsert
        ...
        : await this.g.insertEntities([newMem as dbLib2.IEntPerson]);
    } else {
      // update
      ...
      if (!_.isEmpty(diff) || !!diff.congressRoles) {
        ...
        batchUpdate
          ...
          : await this.g.updateEntities([diff as dbLib2.IEntPerson]);
      }
    }
  }
  ...
  public async createRoleFromPropublica (congress: number, bioGuideId: string): Promise<dbLib2.IEntPersonRole> {
    ...
    let newRole: dbLib2.IEntPersonRole = {
      ...
      state: role.state,
      startDate: this.dateToTimestamp(role.start_date),
      endDate: this.dateToTimestamp(role.end_date)
    };
