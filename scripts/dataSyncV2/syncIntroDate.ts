...
export class IntroducedDateSync {
  ...
  public async syncIntroducedDateForAllBills (
  ...
  public async batchSync (bills: dbLib2.IEntBill[]) {
    ...
    for (let i = 0; i < bills.length; ++i) {
      ...
      const introDate = await this.congressGovIntroDateParser.getIntroducedDate(
        path
      );
      if (introDate) {
...
if (require.main === module) {
  ...
  sync
    .init()
    .then(() =>
      sync.syncIntroducedDateForAllBills(
