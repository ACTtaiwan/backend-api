...
export class AllInfoSync {
  ...
  private readonly bcktName = (<any>awsConfig).s3
    .VOLUNTEER_BILLS_STATICINFO_BUCKET_NAME;
  private readonly bckt = <s3Lib.BillStaticInfoBucket>(
  ...
  public async batchSyncForCongress (
    congress: number,
    attrNames: (keyof dbLib2.IEntBill)[]
  ) {
  ...
  public async patchSingleBill (billId: string) {
    ...
    let attrNames: (keyof dbLib2.IEntBill)[] = [
      'actions',
      'actionsAll',
      's3Entity',
    ];
  ...
  public async syncForBill (
    bill: dbLib2.IEntBill,
    attrNames: (keyof dbLib2.IEntBill)[]
  ) {
    const fLog = this.logger.in('syncForBill');
    const hasAttr = (key: keyof dbLib2.IEntBill): boolean =>
      _.includes(attrNames, key);
    ...
    if (allInfo) {
      ...
      if (hasAttr('s3Entity')) {
        if (allInfo.summaryLatest || allInfo.summaryAll) {
          ...
          let obj = await this.bckt.getEntity(
            bill.congress,
            bill.billType,
            bill.billNumber
          );
          !obj && fLog.log(`Did not find exisitng S3 object.`);
          const url = this.bckt.s3FullUrl(
            bill.congress,
            bill.billType,
            bill.billNumber
          );
      ...
      if (!_.isEmpty(updateBill)) {
        try {
          fLog.log(
            `Writing to database. Object size = ${
              JSON.stringify(updateBill).length
            }`
          );
          updateBill._type = dbLib2.Type.Bill;
          updateBill._id = bill._id;
          fLog.log(
            `Writing to database. Object = ${JSON.stringify(
              updateBill,
              null,
              2
            )}`
